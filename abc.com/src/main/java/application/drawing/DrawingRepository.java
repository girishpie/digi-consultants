...
public class DrawingRepository {
    ...
    private final Logger logger = LoggerFactory.getLogger(DrawingRepository.class);
    @Autowired
    public DrawingRepository (GridFsTemplate gridFsTemplate){
        this.gridFsTemplate = gridFsTemplate;
    }

   public String storeDrawing (final String documentDisplayName,
    ...
    public Drawing getDrawingStream (String drawingId) {
    	Drawing drawing;
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        GridFSDBFile gridFsDbFile = gridFsTemplate.findOne(Query.query(GridFsCriteria.where("_id").is(drawingId)));
        ...
        if (created.isPresent()) {
            try {
                ...
                drawing = new Drawing();
                drawing.setOs(os);
                drawing.setContentType(created.get().getContentType());
                return drawing;   
            } catch (IOException e) {
                e.printStackTrace();
